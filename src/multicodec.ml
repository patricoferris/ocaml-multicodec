...
let softhash_to_code = function `Iscc -> 0xcc01
...
let to_code : t -> int = function
  ...
  | #softhash as x -> softhash_to_code x
  | #transport as x -> transport_to_code x
  | #varsig as x -> varsig_to_code x
  | #zeroxcert as x -> zeroxcert_to_code x

let cid_of_code = function
  | 0x01 -> Some `Cidv1
  | 0x02 -> Some `Cidv2
  | 0x03 -> Some `Cidv3
  | _ -> None
...
let namespace_of_code = function
  ...
  | 0xb39910 -> Some `Subspace_ns
  | 0xb49910 -> Some `Kumandra_ns
  | _ -> None
...
let softhash_of_code = function 0xcc01 -> Some `Iscc | _ -> None
...
let softhash_to_string = function `Iscc -> "iscc"
...
let namespace_of_string = function
  ...
  | "subspace-ns" -> Some `Subspace_ns
  | "kumandra-ns" -> Some `Kumandra_ns
  | _ -> None
...
let softhash_of_string = function "iscc" -> Some `Iscc | _ -> None
